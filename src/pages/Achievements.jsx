  ...
  if (!user) {
    return (
      <div className="text-center py-16">
        <p className="text-ink-500 dark:text-ink-300">බැජ් බලන්න පිවිසෙන්න</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in-up">
      <header className="text-center mb-12">
        ...
        <p className="text-ink-500 dark:text-ink-300 font-medium tracking-wider uppercase text-xs">
          පිරිසිදු අයිකන සහ ලෝගෝ පද්ධතිය
        </p>
        <div className="mt-4 inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-sipyaya-100 dark:bg-sipyaya-900/60 dark:ring-2 dark:ring-sipyaya-500/40 text-sipyaya-700 dark:text-sipyaya-200 shadow-lg dark:shadow-sipyaya-500/10">
          <span className="text-2xl">🏆</span>
          <span className="text-xl font-black">{formatPoints(totalScore ?? 0)}</span>
          <span className="text-sm font-medium">ලකුණු</span>
        </div>
      ...
      {nextBadge && (
        <div className="mb-10 p-4 rounded-xl glass border border-ink-200/60 dark:border-ink-700/60 dark:ring-1 dark:ring-sipyaya-500/20">
          <p className="text-sm text-ink-600 dark:text-ink-300 mb-2">
            ඊළඟ බැජ් එක ලබා ගැනීමට <strong className="text-sipyaya-600 dark:text-sipyaya-200 font-bold">{formatPoints(nextBadge.threshold - (totalScore ?? 0))}</strong> ලකුණු තව ඉතිරි
          </p>
      ...
      <div
        className={`rounded-2xl p-6 sm:p-8 bg-slate-900 dark:bg-slate-950 dark:ring-1 dark:ring-slate-600/50 ${
          viewMode === 'card'
      ...
      <footer className="mt-16 text-center">
        <p className="text-ink-500 dark:text-ink-400 text-[10px] font-black uppercase tracking-[0.8em] opacity-60">
